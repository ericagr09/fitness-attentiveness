...
```{r}
LRS_model1 <- glm.nb(LRS ~ scaled_lat + mast_encounter1 + scaled_lat*mast_encounter1 + scaled_long + exp, data = LRS) 
summary(LRS_model1)
```

```{r}
LRS_model2 <- glm.nb(LRS ~ percent_return + mast_encounter1 + percent_return*mast_encounter1 + scaled_long + exp, data = LRS) 
summary(LRS_model2)
```

# LRS Supplement Analyses (added birth year random effect): 

```{r}
LRS_modelS1 <- glmer.nb(LRS ~ scaled_lat + mast_encounter1 + scaled_lat*mast_encounter1 + scaled_long + exp + (1 | mother_byear), data = LRS) 
summary(LRS_modelS1)
```

```{r}
LRS_modelS2 <- glmer.nb(LRS ~ percent_return + mast_encounter1 + percent_return*mast_encounter1 + scaled_long + exp + (1 | mother_byear), data = LRS) 
summary(LRS_modelS2)
```
# ARS Analyses: 
